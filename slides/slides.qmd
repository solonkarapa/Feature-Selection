...
-   Datasets often have many features—but not all are useful.
-   Some features can be redundant or noisy.
-   Key question: How do we achieve a "good" selection of features?
...
-   $y ∈ ℝ^n$ — response vector (regression)
-   $y ∈ \{0,1\}^n$ — response vector (binary classification)
-   $f(x)$ — prediction function (model)
-   $ŷ_i = f(x_i)$ — predicted response for observation $i$
:::
::::
...
-   Odds-ratio and Chi-Square Test - used for categorical features (eg [epitools::oddsratio()](https://cran.r-project.org/web/packages/epitools/index.html))
-   Mutual Information - measures dependency between features and outcome (eg [FSelectorRcpp](https://cran.r-project.org/web/packages/FSelectorRcpp/index.html))
-   ...
...
-   By having this the penalization term, the lasso forces regression coefficients to become smaller and shrink to zero.
...
1.  Always start with data curation steps:
    -   eg consider removing constant / near-zero variance features.
...
4.  If a (non-)linear embedded method has good predictive performance, then we could proceed to a wrapper method that is combined with a (non-)linear model.
5.  If multiple approaches select large feature sets then this may imply that reducing the number of features may not be feasible.
6.  **Most important**: Feature selection must happen inside resampling, not before it.
7.  **Stability matters more than importance**: Good feature sets should be consistent across folds and not wildly changing ranking.
8.  **There is no single best feature selection method.**
...
-   Liu, H. (2011). [Feature Selection](https://link.springer.com/rwe/10.1007/978-0-387-30164-8_306). In: Sammut, C., Webb, G.I. (eds) Encyclopedia of Machine Learning.
